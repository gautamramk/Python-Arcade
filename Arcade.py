...
class scoreblock():
    ...
    def update(self,kprev,key):
        if (key[K_w] == True and kprev[K_w] == False) or (key[K_w] and kprev==None):
            if ord(self.txt)>=90:
                self.txt = chr(65)
            else:
                self.txt = chr(ord(self.txt)+1)
        elif (key[K_s] == True and kprev[K_s] == False) or (key[K_s] and kprev==None):
            if ord(self.txt)<=65:
                self.txt = chr(90)
            else:
                self.txt = chr(ord(self.txt)-1)
...
class scorescreen():
    def __init__(self,x,y,boxes,screen):
        ...
        self.y = y
        self.boxes = boxes
        self.txtboxes = [scoreblock(x+i*96,y,"A",False,screen) for i in range(boxes)]
...
def main():
    ...
    running = True
    initialize()
    mainmenu = Menu.menulist(200,280,["Snake","Tic Tac Toe","Flappy Birds","Highscores","Exit"],48,48,screen)
    while running:
        screen.fill((128,128,128))
        keystate = pygame.key.get_pressed()
        mainmenu.update(keystate,k_prev)
        mainmenu.draw()
        if(keystate[K_RETURN] == True and (k_prev[K_RETURN]==False or k_prev==None)):
