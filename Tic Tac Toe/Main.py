...
def playcomp():
    ...
    times.append(time.time())
    gamechoose = True
    signchoice = Menu.menulist(100,400,["I choose X","I choose O"],48,48,screen)
    fps = 20
    comp = 0
    while gamechoose:
    ...
    while running:
        if times[0] - times[1] >= (1.0/fps) or firstloop:
            ...
            screen.blit(highlight,(100+curpos%3*80+(5*(curpos%3)),100+(curpos/3)*80+(5*(curpos/3))))
            ...
            for i in range(3):
                for j in range(3):
                    if board[i][j] == -1:
                        screen.blit(crosspic,(100 + j*80+j*5,100+i*80+i*5))
                    elif board[i][j] == +1:
                        screen.blit(naughtpic,(100 + j*80+j*5,100+i*80+i*5))
            c = gameover(board)
            if c[0] == True:
                if c[1] == comp:
                    print "Computer wins."
...
def main():
    ...
    running = True
    firstloop = True
    gamechoice = Menu.menulist(100,400,["Play Against Computer","Play Against Human"],48,48,screen)
    times = []
    keys = []
    keys.append(None)
    keys.append(None)
